...
const AdminDashboard = () => {
    ...
    return (
        <div className="flex min-h-screen bg-[#1B2412] font-sans selection:bg-[#D4AF37]/20 text-[#F0EEE8]">
            ...
            <main className="flex-1 min-w-0 flex flex-col h-screen overflow-hidden">
                {/* Topbar */}
                <header className="h-24 bg-[#1B2412]/80 backdrop-blur-3xl border-b border-white/5 flex items-center justify-between px-8 shrink-0">
                    ...
                    <div className="flex items-center gap-6">
                        <button className="relative w-12 h-12 flex items-center justify-center rounded-xl bg-white/5 text-[#8E8C84] hover:bg-white/10 hover:text-white transition-all">
                            <Bell size={20} />
                            <span className="absolute top-3.5 right-3.5 w-2.5 h-2.5 bg-[#D4AF37] rounded-full border-2 border-[#1B2412]"></span>
                        </button>
                                            ...
                                            <tbody className="divide-y divide-white/[0.03] font-medium">
                                                {users.filter(u => u.name.toLowerCase().includes(searchQuery.toLowerCase())).map(u => (
                                                    <tr key={u.id} className="hover:bg-white/[0.02] transition-colors group">
                                                        ...
                                                        <td className="px-8 py-6 text-center">
                                                            <div className={`w-2 h-2 rounded-full mx-auto animate-pulse ${u.active !== false ? 'bg-[#D4AF37] shadow-[0_0_12px_rgba(0, 240, 255,0.6)]' : 'bg-red-500 shadow-[0_0_12px_rgba(239,68,68,0.6)]'}`}></div>
                                                        </td>
            ...
            <AnimatePresence>
                {isUserModalOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-xl">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#1B2412] border border-white/10 rounded-[2.5rem] w-full max-w-lg p-10 shadow-2xl overflow-hidden relative">
                            <div className="flex items-center justify-between mb-8">
